 ...
 * @author bailarina77
 */
public class registrarConsumoZ extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet registrarConsumoZ</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet registrarConsumoZ at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
            int id_usuario, id_adiccion, gasto_consumo, unidad,cantidad_consumo, consumo_final;
            ...
            if(unidad==1){
                consumo_final= cantidad_consumo*234;
            }
            
            
            if(unidad==2){
                consumo_final= cantidad_consumo*205;
            }
            
            if(unidad==3){
                consumo_final= cantidad_consumo*377;
            }
            
            if(unidad==4){
                consumo_final= cantidad_consumo*150;
            }
            
            if(unidad==5){
                consumo_final= cantidad_consumo*504;
            }
            ...
            int estatus=AccionesConsumo.registrarConsumo(e);
             ...
             if(estatus>0){
                 if (id_adiccion==1){
                     response.sendRedirect("JSP/azucar/consumoZ.jsp");
                 }
